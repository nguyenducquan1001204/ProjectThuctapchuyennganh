...
<div class="student-group-form mb-4">
    <form method="GET" action="{{ route('accounting.systemuser.index') }}">
        <div class="row">
...
<div class="modal fade systemuser-modal" id="createSystemUserModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('accounting.systemuser.store') }}" method="post" id="createSystemUserForm" enctype="multipart/form-data">
                @csrf
    ...
    $(document).ready(function() {
        ...
        document.addEventListener('click', function(e) {
            if (e.target.closest('.edit-systemuser-btn')) {
                ...
                form.action = '{{ route("accounting.systemuser.update", ":id") }}'.replace(':id', userId);
        ...
        document.addEventListener('click', function(e) {
            if (e.target.closest('.delete-systemuser-btn')) {
                ...
                form.action = '{{ route("accounting.systemuser.destroy", ":id") }}'.replace(':id', userId);
    ...
    async function validateEmailInput(input, formType) {
        ...
        if (formType === 'create') {
            try {
                ...
                const response = await fetch('{{ route("accounting.systemuser.checkEmail") }}', {
                    method: 'POST',
                    headers: {
