...
<div class="student-group-form mb-4">
    <form method="GET" action="{{ route('admin.payrollrun.index') }}">
        <div class="row">
...
<div class="modal fade payrollrun-modal" id="createPayrollRunModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.payrollrun.store') }}" method="post" id="createPayrollRunForm">
                @csrf
...
<script>
    ...
    function loadBaseSalaries(unitId, formType) {
        ...
        fetch(`{{ route('admin.payrollrun.getBaseSalariesByUnit', ':unitId') }}`.replace(':unitId', unitId))
    ...
    $(document).ready(function() {
        ...
        document.addEventListener('click', function(e) {
            if (e.target.closest('.edit-payrollrun-btn')) {
                ...
                form.action = '{{ route("admin.payrollrun.update", ":id") }}'.replace(':id', payrollRunId);
        ...
        document.addEventListener('click', function(e) {
            if (e.target.closest('.delete-payrollrun-btn')) {
                ...
                form.action = '{{ route("admin.payrollrun.destroy", ":id") }}'.replace(':id', payrollRunId);
