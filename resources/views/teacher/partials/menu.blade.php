...
@if(isset($teacherMenuItems) && $teacherMenuItems->count() > 0)
    @foreach($teacherMenuItems as $menuItem)
        @php
            ...
            if ($menuItem->externalUrl) {
                ...
                if (Route::has($routeName)) {
                    ...
                    $isActive = $currentRoute === $routeName || 
                                ($menuItem->routeName === 'dashboard' && $currentRoute === 'teacher.dashboard');
                } else {
                    // Route không tồn tại
                    $url = '#';
                }
                        ...
                        @php
                            ...
                            if ($child->externalUrl) {
                                ...
                                if ($childRouteName === 'dashboard') {
                                    $childRouteName = 'teacher.dashboard';
                                }
                                ...
                                if (Route::has($childRouteName)) {
                                    ...
                                    $childActive = $currentRoute === $childRouteName || 
                                                  ($child->routeName === 'dashboard' && $currentRoute === 'teacher.dashboard');
                                } else {
                                    // Route không tồn tại
                                    $childUrl = '#';
                                }
...
@else
    ...
    <li class="{{ request()->routeIs('teacher.dashboard') ? 'active' : '' }}">
        <a href="{{ route('teacher.dashboard') }}">
            <i class="fas fa-home"></i> <span>Bảng điều khiển giáo viên</span>
        </a>
    </li>
@endif
